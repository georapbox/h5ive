...
module.exports = (function () {
    ...
    return function userMedia(opts,successCB,failedCB) {
        ...
        function stream(cb) {
            if (!success_cb) {
                success_cb = cb;
            } else if (success_cb === true) {
                cb.apply(userMedia, success_args);
            } else {
        ...
        function failed(cb) {
            if (!failed_cb) {
                ...
                cb.apply(userMedia, failed_args);
            } else {
                throw new Error('Failed callback already defined.');
            }
        ...
        function handleSuccess() {
            ...
            if (success_cb && typeof success_cb === 'function') {
                success_cb.apply(userMedia, args);
            } else {
                success_cb = true;
        ...
        function handleFailure() {
            if (failed_cb && typeof failed_cb === 'function') {
                failed_cb.apply(userMedia, arguments);
            } else {
                failed_cb = true;
